...
contextBridge.exposeInMainWorld('electron', {
  ...
  tavernExportAllCharacters: (input) => ipcRenderer.invoke('tavern:exportAllCharacters', input || {}),
  ...
  // v1.2.1 批次 12：桌面宠物域（主窗设置/托盘 + 宠物窗口自身共用）
  petToggle: () => ipcRenderer.invoke('pet:toggle'),
  petHide: () => ipcRenderer.invoke('pet:hide'),
  petShow: () => ipcRenderer.invoke('pet:show'),
  petSetClickThrough: (on) => ipcRenderer.invoke('pet:setClickThrough', !!on),
  petSetAlwaysOnTop: (on) => ipcRenderer.invoke('pet:setAlwaysOnTop', !!on),
  petSetRoamMode: (mode) => ipcRenderer.invoke('pet:setRoamMode', mode === 'fixed' ? 'fixed' : 'free'),
  petSetDragging: (on) => ipcRenderer.invoke('pet:setDragging', !!on),
  petMoveTo: (x, y) => ipcRenderer.invoke('pet:moveTo', { x, y }),
  petResetPosition: () => ipcRenderer.invoke('pet:resetPosition'),
  petSetPet: (id) => ipcRenderer.invoke('pet:setPet', id),
  petSetScale: (s) => ipcRenderer.invoke('pet:setScale', s),
  petList: () => ipcRenderer.invoke('pet:list'),
  petState: () => ipcRenderer.invoke('pet:state'),
  petImport: () => ipcRenderer.invoke('pet:import'),
  petDelete: (id) => ipcRenderer.invoke('pet:delete', id),
  petStateChanged: (patch) => ipcRenderer.send('pet:stateChanged', patch),
  onPetAgentEvent: (cb) => ipcRenderer.on('pet:agentEvent', (e, p) => cb(p)),
  onPetSelect: (cb) => ipcRenderer.on('pet:select', (e, id) => cb(id)),
  onPetScale: (cb) => ipcRenderer.on('pet:scale', (e, s) => cb(s)),
  onPetRoam: (cb) => ipcRenderer.on('pet:roam', (e, p) => cb(p)),
});
