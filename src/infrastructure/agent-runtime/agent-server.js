...
const { detectAdapter, buildRequest, parseNonStream, normalizeUsage, parseSSE } = require('../model-gateway/adapters'); // v2（P2-7）
...
const SYSTEM_PROMPT = require('../../core/models/agent-prompt').SYSTEM_PROMPT;
...
const SUBAGENT_TOOLS = {
  explore: ['get_repo_map', 'read_file', 'read_files', 'get_file_outline', 'list_dir', 'glob', 'grep'],
  test: ['get_repo_map', 'read_file', 'list_dir', 'glob', 'grep', 'detect_verification', 'run_tests', 'run_lint', 'run_typecheck'],
  review: ['get_repo_map', 'read_file', 'list_dir', 'glob', 'grep', 'git_status', 'git_diff', 'git_log', 'git_changed_files'],
};
    ...
    if (r.toolCalls && r.toolCalls.length) {
      ...
      for (const tc of r.toolCalls) {
        ...
        const result = await runTool(tc.name, args, ctx.cwd, childEmit, subId, true, () => !!(ctx.aborted && ctx.aborted()), {
          ...
          approveTools: [], cmdWhitelist: [], planMode: !!ctx.planMode, permCtx: ctx.permCtx, workspace: ctx.workspace || null, rootId: ctx.rootId || '', allowedRootIds: ctx.allowedRootIds || [],
        });
        toolCalls++;
  ...
  try {
    if (name === 'run_command') {
      ...
      if (args.session) {
        ...
        const isWin = process.platform === 'win32';
        const child = spawn(isWin ? 'cmd' : 'sh', [isWin ? '/c' : '-c', command], { cwd, windowsHide: true });
        const sess = { child, logs: '', cursor: 0, desc: String(args.description || command), code: null };
...
async function callLLMStream({ apiBase, apiKey, model, messages, thinkLevel, thinkType, tools, promptCaching }) {
  ...
  if (adapter !== 'openai') {
    ...
    const calls = new Map();
    let adapterUsage = { inputTokens: 0, outputTokens: 0, reasoningTokens: 0, cacheReadTokens: 0, cacheWriteTokens: 0 };
    const consume = (line) => {
  ...
  const payload = { model, stream: true, messages, tools: tools || TOOLS, tool_choice: 'auto' };
...
function handleAgent(req, res, body) {
  ...
  const usage = { tokens: 0, toolCalls: 0, steps: 0, repeatedReads: 0, failures: 0, approvals: 0, compressions: 0, inputTokens: 0, outputTokens: 0, reasoningTokens: 0, cacheReadTokens: 0, cacheWriteTokens: 0, estimatedCost: 0, unrelatedFileWrites: 0 };
  ...
  let aborted = false;
  const runAbort = new AbortController(); // v2（脚本隔离）：连接关闭时触发，供 SkillRunner 取消整个进程树
  req.on('close', () => {
    aborted = true;
    try { runAbort.abort(); } catch (_) {}
  ...
  emit('meta', { runId, cwd, auto, planMode: runPlanMode, permissionMode });
  ...
  (async () => {
    ...
    while (segmentIndex < MAX_SEGMENTS) {
      ...
      for (let step = 0; step < maxSteps; step++) {
        ...
        if (aborted) {
          emit('error', terminalPayload({ message: '连接已断开' }));
      ...
      usage.cacheReadTokens = (usage.cacheReadTokens || 0) + Number(r.adapterUsage && r.adapterUsage.cacheReadTokens || 0);
      usage.cacheWriteTokens = (usage.cacheWriteTokens || 0) + Number(r.adapterUsage && r.adapterUsage.cacheWriteTokens || 0);
      ...
      if (r.toolCalls && r.toolCalls.length) {
        ...
        for (const tc of r.toolCalls) {
          ...
          if (!attribution.allowed) {
            ...
            emit('tool_call', { id: tc.id, name: tc.name, args, skillContext: attribution });
          ...
          const raw = await runTool(tc.name, args, cwd, emit, runId, auto, () => aborted, {
    ...
    if (runStore) { try { runStore.updateAgentRun(runId, { status: 'failed', phase: 'failed', usage, error: String(e && e.message ? e.message : e).slice(0, 500), finishedAt: Date.now() }); } catch (e2) {} }
  })
    .finally(() => {
