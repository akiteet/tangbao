...
const { createToolRuntime, TOOL_DEFINITIONS } = require('./tool-runtime'); // 工具协议定义自 v1.1.5 起归 tool-runtime.js（原 LEGACY_TOOL_DEFINITIONS 正名）
...
const TOOL_REGISTRY_VERSION = '1.1.7';
...
// CORS / JSON / SSE / 读体的传输层实现在 agent-server-http.js（v1.1.5 批次 D1 抽出）；
// 以 getter 注入 ALLOW_ORIGIN，保留其运行时可变语义。
const httpLayer = require('./agent-server-http');
const { createAgentHttpServer } = httpLayer;
const cors = (res) => httpLayer.cors(res, () => ALLOW_ORIGIN);
const sendJSON = (res, code, obj) => httpLayer.sendJSON(res, code, obj, () => ALLOW_ORIGIN);
const sseHead = (res) => httpLayer.sseHead(res, () => ALLOW_ORIGIN);
...
// 网页搜索供应商（Tavily / Bing / DuckDuckGo 与编排 doSearch）自 v1.1.5 批次 D3 拆至 search-providers.js
const { webSearch: doSearch } = require('./search-providers');
...
// 业务路由表（入口守卫链——回环 Host / Origin / 启动令牌——与异常兜底在
// agent-server-http.js 的 createAgentHttpServer 中，v1.1.5 批次 D1 抽出）
async function onRoute(req, res, url) {
