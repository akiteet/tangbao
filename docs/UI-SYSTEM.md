...
| `.queue-card` + `.st-*`/`.qc-*` | 任务记录紧凑横条（状态点+名称+详情/失败原因，单行 ellipsis）；糖绘生成队列与糖读工作记录共用同一套类 |
| `.mask` | 弹窗遮罩：纯色半透明（无模糊） |
...
WARN（暂不失败，打印基线供逐模块清零）：非令牌 `border-radius` 字面值、组件层硬编码 hex、间距 px 字面值行数。`--glass*`/`--modal-bg` 重映射别名已全部退役（v1.2.0），引用并入死令牌 FAIL 规则；新增令牌引用一律走语义名，不要一次全文件替换。
