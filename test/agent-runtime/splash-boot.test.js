...
test('splash.html 像素字标自包含（手写点阵字形/无logo无进度条/淡出钩子/背景模式）', () => {
  ...
  // 2026-08-30 用户亲手重写 splash 定稿：断言对齐用户实现（精炼变量名/内联参数），保留原设计意图
  assert.ok(src.includes('aria-label="TangBao 启动中"'), '无障碍标注存在');
  assert.ok(src.includes('var W="Tangbao"') && src.includes('r:["XXXXX"'), '手写几何点阵字形表 + Tangbao 组词（非字体栅格化）');
  assert.ok(src.includes('id="fallback"') && src.includes('textContent="TangBao"'), 'canvas 不可用时降级为纯文字');
  ...
  assert.ok(src.includes('bg=solid') && src.includes('html.solid'), 'solid 模式铺底色类（不支持 acrylic 的系统不发白）');
  assert.ok(!/<script[^>]+src=/.test(src), '无外部脚本依赖（file:// 自包含）');
});

test('对比度与闪烁特效：中性近纯色底 + 内边缘流光 + 放大字标 + 主题色低强度参与', () => {
  const src = read('splash.html');
  // 2026-08-30 用户定稿对齐：视觉决策不变，断言串随用户精炼实现更新
  assert.ok(!src.includes('115deg'), '斜向光斑已删（大范围柔和渐变显脏）');
  assert.ok(!src.includes('--veil') && !src.includes('--band-hi') && !src.includes('--band-lo'), '玻璃轻纱与通宽硬边暗带已删');
  ...
  assert.ok(src.includes('conic-gradient') && src.includes('splash-border-flow') && src.includes('-webkit-mask-composite'), '内边缘流光：mask 打孔环线旋转，不遮字标');
  assert.ok(src.includes('setProperty("--splash-accent"'), '流光颜色经 --splash-accent 注入加亮主题色');
  assert.ok(src.includes('accent=([0-9a-fA-F]{6})') && src.includes('(255-z)*.22'), '主题色解析并向白提亮 22%（流光与光标用）');
  assert.ok(!src.includes('rgba(12, 14, 18') && !src.includes('rgba(8, 10, 14') && !src.includes('#111214'), '蓝移调黑清零（与应用中性灰同族）');
  assert.ok(src.includes('COL=cols+1') && src.includes('CROWS=7') && src.includes('CPER=1060'), '尾随终端式方块光标（主题元素）');
  ...
  assert.ok(src.includes('Math.min(540/') && src.includes('Math.min(15,'), '字标放大到 15px 砖格（更有气势）');
  assert.ok(src.includes('138+(255-138)'), '横向灰→白渐变端点（左端提亮保证与背景对比）');
  assert.ok(src.includes('cxs.length*.14') && src.includes('900+Math.random()*720'), '呼吸灯收敛为低密度动效并保持错峰');
  assert.ok(src.includes('SWW=2') && src.includes('SWD=880'), '一次窄亮带扫光');
  assert.ok(src.includes('h.r[0].length+(i?1:0)'), '字母间保留规整空列');
  assert.ok(src.includes('.72+.28*'), '呼吸最低亮度保持稳定可读');
});
